 ...
 */
  ...
  const changeValues = (text) => {
    ...
    text === "كل الأخبار"
      ? setPostsToShow(posts.blog_post)
      : setPostsToShow(posts.blog_post.filter((item) => item.post_category === text));
  };
  ...
  return (
    <section className="cer newsItems">
      <div className="section-branding">
        ...
        <div>
          <DropdownButton id="dropdown-basic-button" title={dropdownTitle.title}>
      ...
      <div className="newsItemsCardsGrid">
        ...
        {postsToShow ? postsToShow.map((item) => (
          <Card className="grid-col-span-4 newsPageForImg" key={item.id}>
            ...
            <Card.Body>
              ...
              <Link
                to={{
                  pathname: `/post/${item.id}-${item.post_title.trim().split(' ').join('-')}`,
                  state: {
                    id: item.id,
        ...
        Array.from({length: 6}).map((_, index) => (
          ...
          <Card.Body>
            <Card.Title as="h3"><Skeleton /></Card.Title>
            <Card.Text>
              <Skeleton />
            </Card.Text>
            <Skeleton />
