    ...
    <section className="cer">
      <div className="section-branding flex-wrap">
        <div className="heading">
          <img src={turkingFlagImg} alt="Syria Exchange" className="logo" style={{maxWidth: '32px'}}/>
          <h2 className="title" style={{ color: color }}>{title}</h2>
        </div>

        <time className="date" datetime={tryGold?.last_update}>{tryGold?.last_update ? formatDate(new Date(tryGold.last_update)) : <Skeleton width={120} style={{opacity: '0.2'}} />}</time>
      ...
      <div className="goldPricesToast">
        ...
        {TryGold ? TryGold[0].map((item, index) => (
