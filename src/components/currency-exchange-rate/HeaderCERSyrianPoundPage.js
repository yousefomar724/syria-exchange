...
const HeaderCERSyrianPoundPage = ({ city }) => {
  ...
  let localCityName =
    localStorage.city_name === undefined
  ...
  return (
    <section className="cer">
      <div className="section-branding flex-wrap">
        ...
        <time className="date" datetime={cityCoins?.last_update}>{cityCoins?.last_update ? formatDate(new Date(cityCoins.last_update)) : <Skeleton width={120} style={{opacity: '0.2'}} />}</time>
      </div>
